...
const RandomValueProvider = (): IRandomValueProvider => {
  ...
  function seed() {
    const s = 2 * Math.floor(isaac.random() * Math.pow(2, 31)) - 1;
    if (s < 2) return seed();
    else return s;
  }
  ...
  function bbs(n) {
    ...
    const p1 = Math.floor(isaac.random() * 2) + 25; // first power, 25 or 26
    const p2 = 51 - p1; // second power
  ...
  return {
    getRandomString(size: number) {
      ...
      size = size || 40;
      let key = "";
      const shuf_arr = shuffle(digitStr.split(""));
